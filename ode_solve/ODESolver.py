#!/usr/bin/env python3
# Super quick ODE solver implementation to illustrate from scratch
# dynamical system modelling, numerical ODE solutions, and controls
# simulations

# http://hplgit.github.io/primer.html/doc/pub/ode2/ode2-readable.html
...
class ODESolver(object):
    ...
    def solve_single_step_ode(self, dt):
        self.integrator = ode(self.f).set_integrator('lsoda')
        self.integrator.set_initial_value(self.U0, 0)

        if(self.integrator.successful()):
            u = self.integrator.integrate(self.integrator.t+dt)
            self.state.append(u)
            self.set_initial_condition(u)
        return u

    def solve_single_step(self, dt):
        """
        Compute solution to ODE one time step into the future
        """
        #print('solver single step executing')
        T_curr = self.time[-1]
        T = T_curr + dt
        T_in = [T_curr, T]
        u, t = self.solve(T_in, self.terminate)
        #print('u = ', u)

        #print('U = ', u)
        #print('t = ', t)

        self.state.append(u[1,:])
        self.time.append(T)
        #print('Self.time = ', self.time)

        # set initial condition for next iteration as current state
        self.set_initial_condition(u[1,:])

        """
        T = T_curr + dt
        T_in = [T_curr, T]
        u, t = solver.solve(T_in, terminate)
        x = u[1,0]  # or array([x for x, vx, y, vy in u])
        y = u[1,2]  # or array([y for x, vx, y, vy in u])
        xdot = u[1,1]
        ydot = u[1,3]
        xout.append(x)
        yout.append(y)
        ...
        # set next iterations initial conditions and current state
        Uk = [x, xdot, y, ydot]
        solver.set_initial_condition(Uk)
        print('U = ', u)
        print('T_curr = ', T_curr)
        T_curr = T
        """
        # return the system states at time = T_curr + dt
        return u[1,:]

class ODEInt(ODESolver):
    def advance(self):
        #print('advance executing')
        #print('self u = ', self.u)
...
class ODE(ODESolver):
    def advance(self):
        ...
        # vode - real valued variable coefficient ODE solver with
        # fixed leading coefficient implementation.
        # zvode - complex valued variable coefficient ODE solver with
        # fixed leading coefficient implementation
        # lsoda  - real valued variable coefficient ODE solver with
        # fixed leading coefficient implementation.  Automatic method
        # switching between implicit Adams method (for non stiff problems)
        # and method based on BDF (bakward differentiation formulas) for stiff
